...
def load_sheet(path, sheet):
    ...
    if fname not in sheets:        
        #sheets[fname] = skimage.io.imread(fname)
        sheets[fname] = scipy.ndimage.imread(fname, mode="RGBA")
...
def render_map(rmap, tiles, out_img, items, w=16):
    ...
    for item in items:
        ...
        if ch in tiles:
            ...
            alpha = -tiles[ch][:,:,3][:,:,None]
            
            out_img[y*w:(y+1)*w, x*w:(x+1)*w, :] = -(alpha * tiles[ch][:,:,:3] - (1-alpha)*out_img[y*w:(y+1)*w, x*w:(x+1)*w, :])
...
class Map(object):
    ...
    def render(self, extra_items=[]):
        item_list = [(k[0],k[1],v) for k,v in self.items.iteritems()]
...
class Plant(object):
    ...
    def tick(self):
        ...
        self.life -= random.uniform(0.01, 0.03)
